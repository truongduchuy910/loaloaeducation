...
module.exports = {
    broadcast: async function (labels, user, text, url) {
        ...
        if (url) {
            ...
            if (labels.length > 1) {
                var { content_id } = await broadcast.sending_broadcast_messages_with_labe_predicates(attachment_creative_id, labels);
            } else {
                var { content_id } = await broadcast.sending_broadcast_messages_with_labe(attachment_creative_id, labels[0]);
